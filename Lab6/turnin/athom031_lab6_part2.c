...
enum States {Start, lightB0, lightB1, lightB2, lightB1_two, PRS_BUTTN, WAIT_RESET, mirrorlightB0, mirrorlightB1, mirrorlightB2, mirrorlightB1_two, } state;

unsigned char tmpB = 0x00; //tmp variable to hold output
unsigned char tmpA = 0x00; //tmp variable to hold input

//unsigned char buttnREL = 0x01;
void Tick() {
    ...
    switch(state) {
        case Start:
            ...
            break;
        case lightB0:
            state = tmpA ? PRS_BUTTN : lightB1;
            break;
        case lightB1:
            state = tmpA ? PRS_BUTTN : lightB2;
            break;
        case lightB2:
            state = tmpA ? PRS_BUTTN : lightB1_two;
            break;
        case lightB1_two:
            state = tmpA ? PRS_BUTTN : lightB0;
            break;
        case PRS_BUTTN:
            state = tmpA ? PRS_BUTTN : WAIT_RESET;
            break;
        case WAIT_RESET:
            state = tmpA ? mirrorlightB0 : WAIT_RESET;
            break;
        case mirrorlightB0:
            state = tmpA ? mirrorlightB1: lightB1;
            break;
        case mirrorlightB1:
            state = tmpA ? mirrorlightB2 : lightB2;
            break;
        case mirrorlightB2:
            state = tmpA ? mirrorlightB1_two : lightB1_two;
            break;
        case mirrorlightB1_two:
            state = tmpA ? mirrorlightB0 : lightB0;
            break;
        default:
            break;
